...
if (!ISSET($_SESSION['status_master1']) || $_SESSION['status_master1'] != "yes") {
	die("Access Denied");
}
?>
...
//  Matt Brown
//************************************************************************************************************
if (OpenGridForm("", "leave_status_master")) {
...
function DefineFields() {
  ...
  $oForm->AddEntryField("status_id", "ID", "B", "<auto>");
  $oForm->ConfirmDeleteColumn();
  $oForm->SortAsc();
  $oForm->CurrentField["width"]=50;

  $oForm->AddEntryField("status_name", "Status Name", "T", "");
  $oForm->CurrentField["filterUI"]="t";
  $oForm->CurrentField["width"]=150;

  $oForm->AddPanel("New");

  $oForm->AddEntryField("is_new", "New?", "T", "");
  $oForm->CurrentField["width"]=150;

  $oForm->AddEntryField("is_new_default", "New Default", "T", "");
  $oForm->CurrentField["width"]=120;

  $oForm->AddPanel("Approved");

  $oForm->AddEntryField("is_approved", "Approved", "T", "");
  $oForm->CurrentField["width"]=120;

  $oForm->AddEntryField("is_approved_default", "Approved Default", "T", "");
  $oForm->CurrentField["width"]=120;

  $oForm->AddPanel("Rejected");

  $oForm->AddEntryField("is_rejected", "Rejected", "T", "");
  $oForm->CurrentField["width"]=120;

  $oForm->AddEntryField("is_rejected_default", "Rejected Default", "T", "");
  $oForm->CurrentField["width"]=120;

  $oForm->AddEntryField("is_deleted", "Deleted", "T", "");
  $oForm->CurrentField["width"]=120;

  $oForm->AddEntryField("is_deleted_default", "Deleted Default", "T", "");
  $oForm->CurrentField["width"]=120;
