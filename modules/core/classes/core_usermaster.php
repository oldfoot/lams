...
require_once $GLOBALS['dr']."modules/core/classes/core_email.php";
require_once $GLOBALS['dr']."modules/core/classes/core_mq.php";
...
class UserMaster {
	...
	public function Info() {
		...
		$db=$GLOBALS['db'];
		$sql="CALL sp_core_usermaster_browse('".$this->userid."')";
		$this->debug($sql);
		//echo $sql;
		$result = $db->Query($sql);
	...
	public function SetWorkspaceID() {
		if (ISSET($this->workspace_id) && ISSET($this->userid)) {
			$sql = "CALL sp_core_workspace_userrole_browse ('".$this->userid."','".$this->workspace_id."')";
			$this->debug($sql);
			$result = $GLOBALS['db']->Query($sql);
			// ERROR
			if (!$result) {
			...
			if ($GLOBALS['db']->NumRows($result) > 0) {
				//while ($row = $GLOBALS['db']->FetchArray($result)) {
				$this->debug("User exists in the workspace");
				$_SESSION['workspace_id'] = $this->workspace_id;
				//}
			}
			else {
				$this->debug("User does NOT exists in the workspace");
				return false;
			}
		}
		else {
			$this->debug("Invalid params");
			return false;
		}
	}
