...
function CreateDBStoredProcs() {
	$db=$GLOBALS['db'];
	$db->Query("SET FOREIGN_KEY_CHECKS = 0");
	if ($handle = opendir('storedprocs/')) {
		while (false !== ($file = readdir($handle))) {
			if ($file != "." && $file != "..") {
       	$data=file_get_contents("storedprocs/".$file);
       	//echo $data;
       	//echo "<hr>";
       	$db->Query($data);
       }
...
function CreateConfigFile() {
  ...
  $db_setup.="\n";
	$db_setup.=ConfigRow("MyHalo Integration","myhalo_reg_url","http://localhost/m/webservices/rest/register.php?format=xml");
